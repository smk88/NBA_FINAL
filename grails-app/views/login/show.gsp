	...
	<body>
		...
		<div id="show-login" class="content scaffold-show" role="main">
			...
			<ol class="property-list login">
				...
				<g:if test="${loginInstance?.user_type}">
				<li class="fieldcontain">
					<span id="user_type-label" class="property-label"><g:message code="login.user_type.label" default="Usertype" /></span>
					
						<span class="property-value" aria-labelledby="user_type-label"><g:link controller="userType" action="show" id="${loginInstance?.user_type?.id}">${loginInstance?.user_type?.encodeAsHTML()}</g:link></span>
